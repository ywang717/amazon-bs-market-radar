...
function ConvertTo-MarketStructurePostgresLiteral {
    param($Value)
    if ($null -eq $Value) { return 'NULL' }
    return "'" + ([string]$Value).Replace("'", "''") + "'"
}

function New-BestSellersMarketStructureImportSql {
    [CmdletBinding()]
    param([Parameter(Mandatory = $true)][string]$ArtifactPath)
    if (-not (Test-Path -LiteralPath $ArtifactPath -PathType Leaf)) { throw "Market structure artifact not found: $ArtifactPath" }
    $artifact = Get-Content -LiteralPath $ArtifactPath -Raw -Encoding UTF8 | ConvertFrom-Json
    if ([string]$artifact.schema_version -ne 'best-sellers-market-structure-v1') { throw 'Unsupported market structure schema version.' }
    ...
    return @('BEGIN;', 'SET search_path TO amazon_intelligence, public;',
        ("SELECT ingest_best_sellers_market_structure({0}::jsonb);" -f (ConvertTo-MarketStructurePostgresLiteral $json)), 'COMMIT;') -join [Environment]::NewLine
}

function Invoke-BestSellersMarketStructurePostgresImport {
    [CmdletBinding()]
    ...
    try {
        ...
        $env:PGUSER = [string]$PostgresSettings.username; $env:PGDATABASE = [string]$PostgresSettings.database
        $env:PGPASSWORD = [string]$PostgresSettings.password
        (New-BestSellersMarketStructureImportSql -ArtifactPath $ArtifactPath) | & $psql -X --no-psqlrc -v ON_ERROR_STOP=1
        if ($LASTEXITCODE -ne 0) { throw "Market structure PostgreSQL import failed with exit code $LASTEXITCODE" }
    }
    finally {
...
Export-ModuleMember -Function New-BestSellersMarketStructureImportSql, Invoke-BestSellersMarketStructurePostgresImport
