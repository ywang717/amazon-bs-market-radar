[CmdletBinding()]
param([string]$ArchivePath,[switch]$RestoreDatabase,[string]$ModulePath,[scriptblock]$ExitRunner)
$ErrorActionPreference='Stop'
$projectRoot=Split-Path -Parent $PSScriptRoot
if([string]::IsNullOrWhiteSpace($ModulePath)){$ModulePath=Join-Path $projectRoot 'src\LocalPackageHistory.psm1'}
try{
    Import-Module $ModulePath -Force
    $summary=Invoke-LocalPackageHistoryImport -ProjectRoot $projectRoot -ArchivePath $ArchivePath -RestoreDatabase:$RestoreDatabase
}catch{
    ...
    $summary=[pscustomobject]@{status='FAILED';archive_path=$ArchivePath;error=$errorText}
}
